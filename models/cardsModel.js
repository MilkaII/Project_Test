...
function cardFromDB(dbObj) {
    return new Card(dbObj.crd_id,dbObj.crd_name, dbObj.crd_img_url, dbObj.crd_lore, dbObj.crd_description, dbObj.crd_level, dbObj.crd_cost, dbObj.crd_timeout, dbObj.crd_max_usage, dbObj.crd_type);    
}
...
class Card {
    ...
    static async save(newCard) {
        try {
            let [dbCards, fields] = await pool.query("Select * from cards where crd_name=?", [newCard.name]);
            if (dbCards.length)
                return {
                    status: 400, result: [{
    ...
    static async filterByLoreOrDescription(text) {
        try {
            let result = [];
            let [dbCards] =
                await pool.query(`Select * from cards where crd_description LIKE ? or crd_lore LIKE ?`, ['%'+text+'%','%'+text+'%']);
            for (let dbCard of dbCards) {
                result.push(cardFromDB(dbCard));
            }
            return { status: 200, result: result };
    ...
    static async edit(newInfo) {
        try {
            ...
            let [dbCards] = await pool.query("Select * from cards where crd_id=?", [newInfo.id]);
            if (dbCards.length == 0) {
                return {
                    status: 404, result: {
            ...
            [dbCards] = await pool.query("Select * from cards where crd_name=? and crd_id!=?", [newInfo.name, newInfo.id]);
            if (dbCards.length)
                return {
                    status: 400, result: [{
                        location: "body", param: "name",
            ...
            let [result] = await pool.query(`update cards set crd_name=?, crd_img_url=?, crd_lore=?, crd_description=?, crd_level=?, crd_cost=?, crd_timeout=?, crd_max_usage=?, crd_type=? where crd_id=?`, [newInfo.name, newInfo.url, newInfo.lore, newInfo.description, newInfo.level, newInfo.cost, newInfo.timeout, newInfo.maxUsage, newInfo.type, newInfo.id]);
    ...
    static async deleteById(id) {
        try {
            let [result] = await pool.query("delete from cards where crd_id=?", [id]);
